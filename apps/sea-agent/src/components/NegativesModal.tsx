...
import {
  X, Loader2, Sparkles, Download, Upload, Search, Plus, Trash2, AlertCircle, CheckCircle2,
} from 'lucide-react'
...
export default function NegativesModal({ onClose }: Props) {
  ...
  async function generateAI(mode: 'replace' | 'append') {
    ...
    try {
      ...
      setInfo(`AI generated ${json.generated} keywords (${json.inserted} new). Total: ${json.total_in_db}`)
      await reload()
    } catch (e) {
  ...
  async function uploadFile(file: File, mode: 'replace' | 'append') {
    ...
    try {
      ...
      setInfo(`Uploaded ${json.parsed} rows (${json.inserted} new/updated). Total: ${json.total_in_db}`)
      await reload()
    } catch (e) {
  ...
  function downloadXlsx() {
    ...
    XLSX.writeFile(wb, `ibizz-negative-keywords-${stamp}.xlsx`)
  }

  function downloadTemplate() {
    const data = [
      { keyword: 'voorbeeld', match_type: 'broad', category: 'general', notes: 'optioneel' },
    ]
    const ws = XLSX.utils.json_to_sheet(data, { header: ['keyword', 'match_type', 'category', 'notes'] })
    ws['!cols'] = [{ wch: 35 }, { wch: 12 }, { wch: 18 }, { wch: 40 }]
    const wb = XLSX.utils.book_new()
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-gray-900">Negative keywords</h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Global list applied to all SEA campaigns · {items.length} total
            </p>
          </div>
        ...
        <div className="px-6 py-3 border-b border-gray-100 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 disabled:opacity-40"
          >
            <Download size={11} />
            Download .xlsx
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-500 hover:text-gray-800"
          >
            Download template
          </button>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="text-xs border border-gray-200 rounded-lg pl-7 pr-2 py-1.5 outline-none focus:border-[#EB4628] w-44"
              />
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-2 border-b border-gray-100 bg-gray-50 flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 text-xs border border-gray-200 rounded px-2 py-1 outline-none focus:border-[#EB4628] bg-white"
          />
          <Select
            value={newMatchType}
          ...
          <input
            ...
            className="text-xs border border-gray-200 rounded px-2 py-1 outline-none focus:border-[#EB4628] bg-white w-28"
          />
          <button
            onClick={addManual}
            disabled={!newKeyword.trim()}
            className="text-xs font-semibold text-[#EB4628] hover:underline disabled:opacity-40"
          >
            Add
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center py-20 text-center">
              {items.length === 0 ? (
                <>
                  <p className="text-sm font-semibold text-gray-700 mb-1">No negative keywords yet</p>
                  <p className="text-xs text-gray-400 mb-5">Click &quot;Generate AI defaults&quot; to start with a Dutch baseline list (~500 terms)</p>
                </>
              ) : (
      ...
      {confirmReplace && (
        <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center p-4" onClick={() => { setConfirmReplace(null); setPendingFile(null) }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-base font-bold text-gray-900 mb-2">
              Replace existing list or append?
            </h3>
            <p className="text-xs text-gray-500 mb-5">
              You currently have {items.length} keywords. Replace = delete all + insert new. Append = keep existing + add new (duplicates updated).
            </p>
            <div className="flex flex-col gap-2">
              <button
                onClick={() => confirmReplace === 'generate' ? generateAI('append') : pendingFile && uploadFile(pendingFile, 'append')}
                className="py-2.5 rounded-xl text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200"
              >
                Append (safe)
              </button>
              <button
                onClick={() => confirmReplace === 'generate' ? generateAI('replace') : pendingFile && uploadFile(pendingFile, 'replace')}
                className="py-2.5 rounded-xl text-sm font-semibold text-white bg-red-500 hover:bg-red-600"
              >
                Replace all ({items.length} → 0 → new)
              </button>
              <button
                onClick={() => { setConfirmReplace(null); setPendingFile(null) }}
                className="py-2 text-xs text-gray-500 hover:text-gray-800"
              >
                Cancel
              </button>
            </div>
