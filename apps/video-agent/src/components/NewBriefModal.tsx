...
import type { Brand, VideoBrief } from '@ibizz/supabase'
import { useAuth } from '@/lib/auth'

type Props = {
  onClose: () => void
  onCreated: (brief: VideoBrief) => void
}

const BRAND_COLORS = ['#6366f1', '#8b5cf6', '#ec4899', '#EB4628', '#f97316', '#eab308', '#22c55e', '#14b8a6', '#3b82f6', '#64748b']

export default function NewBriefModal({ onClose, onCreated }: Props) {
  const { user, userName } = useAuth()
  const [brands, setBrands] = useState<Brand[]>([])
  const [brandId, setBrandId] = useState<string>('')
  const [dagTitel, setDagTitel] = useState('')
  const [introSubtitel, setIntroSubtitel] = useState('')
  const [overzicht, setOverzicht] = useState('')

  const [creatingBrand, setCreatingBrand] = useState(false)
  const [newBrandName, setNewBrandName] = useState('')
  const [newBrandColor, setNewBrandColor] = useState(BRAND_COLORS[3])
  ...
  // Zodra een brand gekozen is en intro nog leeg: voorvullen met "ibizz × {brand}"
  useEffect(() => {
    if (!introSubtitel && brandId) {
      const brand = brands.find(b => b.id === brandId)
      if (brand) setIntroSubtitel(`ibizz × ${brand.name}`)
    }
  ...
  async function save() {
    if (!dagTitel.trim()) return
    setSaving(true)
    setError(null)
    try {
      const { data, error: err } = await supabase.from('video_briefs').insert({
        brand_id: brandId || null,
        dag_titel: dagTitel.trim(),
        intro_subtitel: introSubtitel.trim() || null,
        overzicht: overzicht.trim() || null,
        status: 'draft',
        versie: 1,
        created_by: user?.id ?? null,
        created_by_name: userName || null,
      ...
      if (data) onCreated(data as VideoBrief)
      onClose()
    } catch (e) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-base font-bold text-gray-900">Nieuwe shooting brief</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <Field label="Dag-titel">
            <input
              autoFocus
              value={dagTitel}
              onChange={e => setDagTitel(e.target.value)}
              placeholder="Bijv. Dag 1 — Op locatie"
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-[#EB4628]"
            />
            <p className="text-[11px] text-gray-400 mt-1">
              Tip: gebruik nummering — dit is hoe de scripts gegroepeerd worden per draaidag.
            </p>
          </Field>

          <Field label="Intro / subtitel">
            <input
              value={introSubtitel}
              onChange={e => setIntroSubtitel(e.target.value)}
              placeholder="Bijv. ibizz × FRENKY"
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-[#EB4628]"
            />
          </Field>

          <Field label="Overzicht (optioneel)">
            <textarea
              value={overzicht}
              onChange={e => setOverzicht(e.target.value)}
              placeholder="Korte beschrijving wat er in deze dag zit. Wordt later automatisch bijgewerkt na script-generatie."
              rows={3}
